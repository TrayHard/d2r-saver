...
export async function magicSuffixToJson(): Promise<Record<string, unknown>> {
  const table = await readGameFile('magicSuffix');
  const magicSuffix: Record<string, unknown> = {};
  let i = 1;

  for (const line of table) {
    if (getString(line[0]).trim() === '') { i++; continue; }
    const code = padCode('ms', i);
    magicSuffix[code] = {
      name: getString(line[0] + ' ').trim() || undefined,
      class: getString(line[8] + ' ').trim() || undefined,
      classlevelreq: getString(line[9] + ' ').trim() || undefined,
  ...
  await writeJson('magicSuffix', magicSuffix);
  return magicSuffix;
}
