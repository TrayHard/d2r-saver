...
export async function magicPrefixToJson(): Promise<Record<string, unknown>> {
  const table = await readGameFile('magicPrefix');
  const magicPrefix: Record<string, unknown> = {};
  let i = 1;

  for (const line of table) {
    if (getString(line[0]).trim() === '') { i++; continue; }
    const code = padCode('mp', i);
    magicPrefix[code] = {
      name: getString(line[0] + ' ').trim() || undefined,
      classspecific: getString(line[7] + ' ').trim() || undefined,
  ...
  await writeJson('magicPrefix', magicPrefix);
  return magicPrefix;
}
