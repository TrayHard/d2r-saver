...
import { parseParamStats, parseUniqueStats as parseUniqueStatsHelper, parseRunewordStats as parseRunewordStatsHelper, parseModStats as parseModStatsHelper, } from '../items/item-stats-parser.js';
// ─── Constants ──────────────────────────────────────────────────
/** Maps the 4-bit quality field from the binary format to our Quality enum values. */
const gameQualityMap = [0, 1, 2, 3, 4, 5, 6, 7, 8];
 ...
 */
function addStat(stats, stat, param, value) {
    if (stat.startsWith('item_') && (stat.endsWith('_skill_onevent') ||
 ...
 */
export function createItemParser(reader, gd, initialId = 0, options) {
    ...
    function itemTypeTreeFilter(type) {
    ...
    function parseItem(id, handler) {
        ...
        if (ear) {
            ...
            items[id] = item;
            emitLocation(id, location, bodyloc, storage, invrow, invcol, item, handler);
            return;
        }
        ...
        if (based !== ' ')
            throw Error(`invalid item code`);
        const baseId = basea + baseb + basec;
        if (!gd.items[baseId])
            throw Error(`invalid item code ${baseId} at bit ${reader.bitpos}`);
        item.base = baseId;
        // Simple item
        if (simple) {
            ...
            if (reader.bits(1))
                item.quantity = reader.bits(8);
            reader.align();
            item.quality = 2; // NORMAL
            item.binaryOffset.end = reader.bitpos;
            if (presetItemIds.has(baseId)) {
                items[baseId] = item;
                emitLocation(baseId, location, bodyloc, storage, invrow, invcol, item, handler);
            }
            else {
                items[id] = item;
                emitLocation(id, location, bodyloc, storage, invrow, invcol, item, handler);
        ...
        switch (qualityRaw) {
            ...
            case 3: // SUPERIOR
                ...
                break;
            case 4: // MAGIC
                if ((modid = reader.bits(11)))
                    mods.push(`mp${mkid(modid)}`);
                if ((modid = reader.bits(11)))
                    mods.push(`ms${mkid(modid)}`);
                break;
            case 5: // SET
            ...
            case 8: // CRAFTED
                {
                    const name1 = gd.rarePrefix[reader.bits(8) - 156]?.name;
                    const name2 = gd.rareSuffix[reader.bits(8) - 1]?.name;
                    item.name = `${gd.locale.strings[name1] || name1 || ''} ${gd.locale.strings[name2] || name2 || ''}`.trim();
                    if (reader.bits(1))
                        mods.push(`mp${mkid(reader.bits(11))}`);
                    if (reader.bits(1))
                        mods.push(`ms${mkid(reader.bits(11))}`);
                    if (reader.bits(1))
                        mods.push(`mp${mkid(reader.bits(11))}`);
                    if (reader.bits(1))
                        mods.push(`ms${mkid(reader.bits(11))}`);
        ...
        if (types.has('armo')) {
            ...
            const baseMaxAc = 'maxac' in base ? base.maxac : 0;
            const baseMinAc = 'minac' in base ? base.minac : 0;
        ...
        if (('stackable' in base && base.stackable) ||
            (forceStackable && !types.has('ques'))) {
            item.quantity = reader.bits(9);
        }
        if (socketed)
            item.sockets = reader.bits(4);
        // Set flags
        ...
        function parseStats() {
            while (true) {
                ...
                if (!stat)
                    throw Error(`unknown item stat code ${statId}`);
        ...
        if (stats.mindamage || stats.secondary_mindamage || stats.item_throw_mindamage) {
            stats.mindamage = Math.max(...[stats.mindamage, stats.secondary_mindamage, stats.item_throw_mindamage].filter(Boolean));
        ...
        if (stats.maxdamage || stats.secondary_maxdamage || stats.item_throw_maxdamage) {
            stats.maxdamage = Math.max(...[stats.maxdamage, stats.secondary_maxdamage, stats.item_throw_maxdamage].filter(Boolean));
        ...
        if (item.quality === 5 || item.quality === 7) {
            // SET or UNIQUE
            item.uniqueValues = parseUniqueStatsHelper(gd, item, src => parseParamStats(gd, stats, src)) ?? undefined;
            ...
            // RUNEWORD
            const parsed = parseRunewordStatsHelper(gd, item, null, src => parseParamStats(gd, stats, src), mods);
            if (parsed) {
            ...
            const parsed = parseModStatsHelper(gd, item, null, null, null, src => parseParamStats(gd, stats, src), mods);
            if (parsed) {
                Object.assign(item, parsed);
            }
            else {
        ...
        if (presetItemIds.has(item.base) && !item.unique) {
            emitLocation(item.base, location, bodyloc, storage, invrow, invcol, item, handler);
        }
    ...
    function emitLocation(id, location, bodyloc, storage, invrow, invcol, item, handler) {
        switch (location) {
            case 0: // STORED
                if (storage === 1)
                    handler(id, 'inventory', invrow * 10 + invcol, item);
                if (storage === 4)
                    handler(id, 'cube', invrow * 3 + invcol, item);
                if (storage === 5)
                    handler(id, 'stash', invrow * stashColumns + invcol, item);
                break;
            case 1: // EQUIPPED
                if (bodyMap[bodyloc])
                    handler(id, 'body', bodyMap[bodyloc], item);
                break;
            case 2: // BELT
    ...
    function parseItemList(handler) {
