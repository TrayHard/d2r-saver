...
const bodyLocMap: Record<string, number> = {
...
const qualityToFileBits: Record<number, number> = {
  1: 1, // LOW
  2: 2, // NORMAL
  3: 3, // SUPERIOR
  4: 4, // MAGIC
...
export function writeItem(
  ...
  if (types.has('weap') || types.has('armo')) {
    ...
    const maxDur = (baseItem?.nodurability || item.stats?.item_indesctructible) ? 0 : ((baseItem?.durability as number) || 0);
  ...
  if (runeword) {
    ...
    if (runewordData) {
      runewordStats(gd, rwStats, runewordData, item.uniqueValues, item as unknown as Record<string, unknown>);
    }

    const baseStats: Record<string, number> = {};
    if (item.auto) modsToStats(gd, baseStats, item.auto, item as unknown as Record<string, unknown>);
    if (item.superior) modsToStats(gd, baseStats, item.superior, item as unknown as Record<string, unknown>);
    if (item.staff) modsToStats(gd, baseStats, item.staff, item as unknown as Record<string, unknown>);
  ...
  switch (qualityRaw) {
    ...
    case 4: { // MAGIC
      ...
      for (const k in item.mods || {}) {
        if (!mp && k.includes('mp')) mp = +k.slice(2);
        else if (!ms && k.includes('ms')) ms = +k.slice(2);
      }
...
function writeItemStats(
  ...
  for (const stat of sortedKeys) {
    if (processed.has(stat)) continue;
    if (stat === 'item_numsockets') continue; // written in dedicated field
...
function getStatId(stat: string, gd: GameData): number {
...
function writeStatValue(
  ...
  p1: string | undefined,
  p2: string | undefined,
  itemStat: { savebits?: number; saveadd?: number; saveparambits?: number },
  isFollowStat: boolean,
): void {
  ...
  if (itemStat.saveadd) value += itemStat.saveadd;
...
function isItemSimple(item: BinaryParsedItem): boolean {
  // Simple items: presetItemIds that aren't uniquified
  if (presetItemIds.has(item.base) && !item.unique) return true;
  // Items explicitly marked simple (quality 2 with no stats)
  if (item.quality === 2 && !item.unique && Object.keys(item.stats || {}).length === 0) return true;
  return false;
}

/** Write N zero bits. */
function writeZeroBits(writer: BitWriter, count: number): void {
...
function writeSocketedItems(
  ...
  if (item.sockets > 0 && item.socketedItems) {
    for (const s of item.socketedItems) {
      ...
      const socketLoc: ItemWriteLocation = { loc: 6, equip: 0, x: 0, y: 0, storage: 0 };
...
function findRareNames(name: string, gd: GameData): { rarePrefix: number; rareSuffix: number } {
  if (!name) return { rarePrefix: 0, rareSuffix: 0 };
  const [pName, sName] = name.split(' ', 2);
  let rarePrefix = 0;
  let rareSuffix = 0;

  for (const [id, v] of Object.entries(gd.rarePrefix as Record<string, { name?: string }>)) {
  ...
  for (const [id, v] of Object.entries(gd.rareSuffix as Record<string, { name?: string }>)) {
    const resolvedName = gd.locale.strings[v.name!] || v.name;
...
export function buildWriteEntries(
  ...
  const socketed = new Set<number | string>();

  // Collect socketed item IDs to exclude them from top-level entries
  for (const item of Object.values(allItems)) {
    if (item?.socketedItems) {
      for (const s of item.socketedItems) {
        if (!presetItemIds.has(String(s))) socketed.add(s);
      }
    }
  }

  const push = (id: number | string | undefined, loc: ItemWriteLocation): void => {
    if (id == null) return;
    if (socketed.has(id)) return;
  ...
  if (merc) {
    ...
    (profile.stash || []).forEach((id, i) =>
      push(id, { loc: 0, equip: 0, x: i % stashColumns, y: (i / stashColumns) | 0, storage: 5 }),
    );
    (profile.belt || []).forEach((id, i) =>
      push(id, { loc: 2, equip: 0, x: i, y: 0, storage: 0 }),
    );
  }
